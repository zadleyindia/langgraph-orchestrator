    ...
    async with LangGraphMemoryClient() as client:
        ...
        if results:
            ...
            for i, result in enumerate(results[:3]):
                ...
                name = result.get('name') or result.get('entity_name') or result.get('entityName') or 'Unknown'
                entity_type = result.get('entityType') or result.get('entity_type') or 'Unknown'
                obs_count = result.get('observationCount', 0)
                print(f"   Result {i+1}: {name} ({entity_type}) - {obs_count} observations")
        else:
